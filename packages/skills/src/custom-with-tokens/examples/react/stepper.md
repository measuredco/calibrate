...
```tsx
import { Icon, Root } from "@measured/calibrate-react";
...
- **Leaf component for the content node.** `Icon` for the done-state checkmark — Calibrate handles its sizing, color inheritance, and a11y. The number and the icon both render unconditionally; CSS reveals the right one for the state via `opacity` + `@keyframes`.
