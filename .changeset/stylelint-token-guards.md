...
- `declaration-property-unit-disallowed-list` for spacing / sizing / font-size properties — including the full sizing family (`block-size`, `inline-size`, `min-*-size`, `max-*-size`, plus legacy `width`/`height`/`min-*`/`max-*`) and `outline-offset`. Disallows absolute lengths (`px`, `pt`, `pc`, `in`, `cm`, `mm`, `Q`) and root-relative units (`rem`, `rlh`) — both bypass the design system's spacing scale. Everything else is allowed by default: typography-relative (`em`, `ch`, `lh`), `%`, viewport-relative (`vw`/`vh`/`vi`/`vb` plus the `s*`/`l*`/`d*` variants), container-query units, and any future CSS unit. `var()` and `calc()` pass through; unitless `0` is always allowed.
