---
"@measured/calibrate-tokens": minor
---

Introduce `@measured/calibrate-tokens` — the design system as a JSON artifact, with its accompanying JSON Schema. For documentation sites, MCP servers, agents, and any tooling that wants tokens as data.

```js
import msrd from "@measured/calibrate-tokens/msrd";
import wrfr from "@measured/calibrate-tokens/wrfr";
import base from "@measured/calibrate-tokens/base";
import schema from "@measured/calibrate-tokens/schemas/v1";
```
